...
describe.each([
  ...
  it("exposes only the public Web connection and permission controls", () => {
    ...
    expect(html).toContain('data-command="public.connect"');
    ...
    expect(html).not.toContain('data-role="pair-form"');
    expect(html).not.toContain('data-role="manual-pairing"');
    expect(html).not.toContain('data-command="disconnect"');
    expect(html).not.toContain("本地服务");
    expect(html).toContain("连接当前 ArchResearch 网页");
