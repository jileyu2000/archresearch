...
| M164 Web/local user-visible parity and extension-only release naming | in_progress | 用户可见功能、同源界面、多方向逐帖逐图、共享 48 图/48 MiB、R2 对象键事件、IndexedDB 本地预览、扩展专属命名、PR #1、两套 Hosted CI 与 `v2.1.3` 扩展专属 Release 均已闭合。私有 R2 桶与三日生命周期已启用，生产 Worker 已部署为 `c7144317-8daa-4e8f-ae57-5ccf79fc8a41`，HTTP、安全头、正式 Turnstile 配置均通过；剩余是恢复系统 Chrome 控制桥后完成线上 1440/390 视觉 smoke。 |
