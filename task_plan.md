...
| M164 Web/local user-visible parity and extension-only release naming | in_progress | 用户可见功能、同源界面、多方向逐帖逐图、共享 48 图/48 MiB、R2 对象键事件、IndexedDB 本地预览、扩展专属命名、PR #1、两套 Hosted CI 与 `v2.1.3` 扩展专属 Release 均已闭合。当前唯一部署阻塞是 Cloudflare 账号尚未启用 R2（API code 10042）；启用后需创建 `archresearch-visual-previews`、设置三日临时对象生命周期、部署并完成线上 smoke。 |
