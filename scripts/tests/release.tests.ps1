...
$workflowContracts = @(
    ...
    @{ Pattern = 'ArchResearch-Windows-x64-Setup-v2\.2\.8\.exe'; Message = "CI must publish the clearly named v2.2.8 Windows installer artifact." }
    @{ Pattern = 'archresearch-chrome-extension-only-v2\.2\.8\.zip'; Message = "CI must keep the clearly named v2.2.8 Chrome extension package separate." }
)
