...
$workflowContracts = @(
    ...
    @{ Pattern = 'ArchResearch-Windows-x64-Setup-v2\.2\.7\.exe'; Message = "CI must publish the clearly named v2.2.7 Windows installer artifact." }
    @{ Pattern = 'archresearch-chrome-extension-only-v2\.2\.7\.zip'; Message = "CI must keep the clearly named v2.2.7 Chrome extension package separate." }
)
