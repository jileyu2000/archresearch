...
$workflowContracts = @(
    ...
    @{ Pattern = 'ArchResearch-Windows-x64-Setup-v2\.2\.6\.exe'; Message = "CI must publish the clearly named v2.2.6 Windows installer artifact." }
    @{ Pattern = 'archresearch-chrome-extension-only-v2\.2\.6\.zip'; Message = "CI must keep the clearly named v2.2.6 Chrome extension package separate." }
)
...
foreach ($readmeContract in @(
    ...
    'ArchResearch-Windows-x64-Setup-v2\.2\.6\.exe',
    'Windows 安装器不会捆绑扩展',
