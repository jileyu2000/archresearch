...
$workflowContracts = @(
    ...
    @{ Pattern = 'ArchResearch-Windows-x64-Setup-v2\.2\.9\.exe'; Message = "CI must publish the clearly named v2.2.9 Windows installer artifact." }
    @{ Pattern = 'archresearch-chrome-extension-only-v2\.2\.9\.zip'; Message = "CI must keep the clearly named v2.2.9 Chrome extension package separate." }
)
