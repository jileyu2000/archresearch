...
ArchResearch 是建筑学生和青年设计师使用的研究工作面。整个视口是一张浅灰制图桌，而不是承载居中应用卡片的空背景：低对比定位网格、裁切线稿与注册角把宽屏余量也纳入工作面。首页像刚铺开的蓝图桌，问题入口醒目、语气轻快、起点容易尝试；进入结果后则回到克制的评图墙，先重述本次任务与可迁移方法，再展开支撑图纸，来源证据在当前图纸旁按需展开。视觉记忆点来自全局制图场、蓝图任务面、少量标记纸、真实图纸比例与小型“来源钉”，不是渐变、玻璃或指标面板。
...
- **结论先于证据墙：** 结果页先标明“本次研究任务 / 演示任务”，再列不超过三个由现有推断字段支持的方法方向；图纸墙明确命名为支撑证据，不生成脱离来源的新结论。
...
- 图纸墙之前固定出现“任务 → 方法方向”摘要。演示数据必须显示“演示任务”，真实运行显示“本次研究任务”；不得让预置结果看起来像用户刚完成的实时研究。
- 方法方向从排序结果中按不同图纸类型保守取前三项，直接展示既有 `inference`，并说明其不是可直接套用的答案。筛选只改变支撑图纸，不重写总方法摘要。
