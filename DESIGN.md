...
ArchResearch 是建筑学生和青年设计师使用的研究工作面。整个视口是一张浅灰制图桌，而不是承载居中应用卡片的空背景：低对比定位网格、裁切线稿与注册角把宽屏余量也纳入工作面。首页像刚铺开的蓝图桌，问题入口醒目、语气轻快、起点容易尝试；进入结果后则回到克制的评图桌，按“任务 → 子问题 → 项目档案 → 多图证据”展开研究，来源证据在当前图纸旁按需展开。视觉记忆点来自全局制图场、蓝图任务面、少量标记纸、真实图纸比例与小型“来源钉”，不是渐变、玻璃或指标面板。

**产品原则**

- **图像先于控制器：** 默认视图把空间留给图纸；筛选、进度、Trace 和导出不得形成永久仪表盘。
- **问题拆解组织结果：** 结果页先显示任务和 3–6 个子问题，再按子问题组织项目档案；用户始终知道当前案例在回答哪一部分。
- **项目档案承载方法：** 每个档案用多张互补图纸说明项目条件、设计机制、可迁移策略和适用边界，不把单张图纸当作完整案例。
- **证据贴着资产：** 来源钉打开当前图纸的证据抽屉；来源事实、可见观察、方法推断、适用边界与权利状态保持分栏和独立命名。
- **渐进披露：** 项目档案先显示方法分析和关键图组；单张图纸只保留类型与证据层级，详细定位始终在一次明确操作内可达。
...
- **关联分析不串题：** 同一项目或图纸支持多个子问题时，每个关联分别保存与展示分析；不得把第一个子问题的结论复制到其他章节。
- **对照产生判断：** 比较视图使用共享维度矩阵，固定比较解决对象、可借鉴方法、图中观察、证据状态和使用边界；禁止只把两张卡片并排。
...
- 首页工作面桌面最大宽度 1600px。蓝图任务启动器由蓝色标题区和内嵌白色输入纸组成；主输入、三种研究方式、资料入口和一个主动作仍属于同一表单，研究深度和具体附件字段渐进展开，不另建设置面板。
- 主问题输入区按桌面 / 平板 / 手机使用 152 / 132 / 108px 最小高度，并继续允许用户垂直拉伸。它是主要思考工作面，不按普通单行研究栏的 48px 高度处理。
- 任务启动器下方采用“常见问题起点 + 最近研究”两栏：`>1380px` 四个问题起点排成一条横向启动带，最近研究约占四分之一；常规桌面为 2×2 起点，`≤860px` 单列。问题起点点击只填入并聚焦问题；最近研究最多 3 条，显示问题、方式、深度、状态和可用图纸数，点击后才加载完整结果。
...
- 研究启动器内的“运行前检查”使用中性平面状态行，分别表达本地服务、当前页面扩展、临时网页权限和待验证的登录页面；不得把供应商余额、保留额度或模型能力做成产品状态、仪表或设置。

### Project dossier、Drawing sheet 与来源钉

- 结果页固定按“任务 → 问题拆解 → 案例章节”展开。演示数据必须显示“演示任务”，真实运行显示“本次研究任务”；不得让预置结果看起来像用户刚完成的实时研究。
- 每个子问题形成一个案例章节，章节内按项目建立档案。项目档案固定交代项目条件、设计机制、可迁移策略和适用边界，并优先组合 2–3 张平面、剖面、分析图或效果图等互补证据。
- 同一项目支持多个子问题时，可以出现在多个章节，但每个章节读取自己的关联分析；不得只按首个子问题归类，也不得沿用另一章节的机制、观察或边界。
- 档案是一个平面结构区段，标题、分析和图组由分隔线与留白组织；禁止用“项目卡片里再嵌图纸卡片”的层层圆角容器，也禁止退化为同尺寸图纸卡墙。
- 图纸保持原始长宽比，`object-fit: contain`；白色图像井使用 12px 圆角和 1px 结构线。图组按互补性和相关度排序，筛选只改变可见图纸，不改写项目分析。
- 单张图纸默认只出现图纸类型和证据层级。操作收在图纸边缘，hover、focus 或选择后才显示次要动作。
...
- “来源支持的事实 / 图中直接可见 / 设计方法推断 / 适用边界 / 权利状态”必须使用固定词汇与独立区段。每条正式事实旁必须有 URL、PDF 页码或图像区域定位；观察不得补写页面未声明的项目事实，推断不得伪装成来源结论。
...
- 比较入口统一命名为“方法对照”。对照表按结果排序保留列顺序，首列固定共享维度；窄屏只允许表格容器横向滚动，并显示明确的滑动提示，页面本身不得横向溢出。
...
- **Do** 用子问题建立案例章节，以项目档案连接条件、机制、迁移策略、边界和多张互补图纸。
