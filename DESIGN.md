...
ArchResearch 是建筑学生和青年设计师使用的研究工作面。整个视口是一张浅灰制图桌，而不是承载居中应用卡片的空背景：低对比定位网格、裁切线稿与注册角把宽屏余量也纳入工作面。首页像刚铺开的蓝图桌，问题入口醒目、语气轻快、起点容易尝试；进入结果后则回到克制的评图墙，图纸是第一层，项目与方法句是第二层，来源证据在当前图纸旁按需展开。视觉记忆点来自全局制图场、蓝图任务面、少量标记纸、真实图纸比例与小型“来源钉”，不是渐变、玻璃或指标面板。
