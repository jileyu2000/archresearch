---
name: "ArchResearch M8"
description: "克制、图像优先的数字建筑评图墙，证据始终贴近图纸。"
colors:
...
# Design System: ArchResearch M8

## Overview

**Creative North Star: “数字建筑评图墙”**

ArchResearch 是建筑学生和青年设计师使用的研究工作面。界面像工作室里一面克制的浅色评图墙：图纸是第一层，项目与方法句是第二层，来源证据在当前图纸旁按需展开。视觉记忆点来自不同长宽比的图纸与小型“来源钉”，不是装饰性卡片、渐变或指标面板。
...
- **克制而高密度：** 采用单一系统无衬线、短标签、细结构线和功能性色彩；密度来自信息组织，不来自缩小字体。
- **熟悉的工具行为：** 使用标准按钮、表单、菜单、抽屉与键盘顺序；一致性高于新奇交互。
- **问题先于结果：** 默认首页只承载问题输入；已完成的历史结果通过“查看上次结果”明确进入，不能在恢复工作区后自动覆盖首页。仅未结束的运行可以直接恢复到进度/结果状态。
...
颜色采用“中性工作面 + 单一蓝图动作色”的受限策略。任何单屏中高饱和色面积不得超过 10%；色彩只表达动作、选择、证据或限制。
...
| `blueprint` / `blueprint-hover` / `blueprint-soft` | 主动作、当前选择与选择底色；不作装饰标题或大面积背景。 |
...
**蓝色稀缺规则。** 同一区域只允许一个蓝色主动作。蓝图蓝同时出现于按钮、标题、边框和图标时，视为失控。
...
- 首页与结果页互斥显示：首页使用“查看上次结果”进入已完成任务，结果页使用“发起新研究”返回问题输入；不得同时堆叠研究输入和旧结果墙。
- 命令式研究栏桌面最大宽度 880px、最小高度 48px。默认只显示一个问题输入和一个主动作；目标、文件和高级模式在同一区域渐进展开。
...
- 输入框使用 `paper`、`line-strong` 和 6px 圆角；error 同时显示图标、边界和具体修复文字。placeholder 按正文对比度处理。
