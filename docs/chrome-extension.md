...
只有“小红书图纸灵感”需要这个扩展。建筑案例研究和本地资料管理可以先直接使用。

Windows 安装包不包含 Chrome 扩展。Chrome Web Store 尚未上架，当前需要手动加载一次：

1. 在 ArchResearch 的扩展提醒中点击“查看安装方法”，下载名称含 `chrome-extension-only` 的 ZIP。
2. 解压 ZIP。需要选择的文件夹根目录应当直接包含 `manifest.json`。
3. 打开 `chrome://extensions`，开启“开发者模式”，点击“加载已解压的扩展程序”，选择上一步的文件夹。
4. 回到 ArchResearch。公共网页从扩展中点击“连接当前 ArchResearch 网页”；本地安装版可在页面中点击“一键连接浏览器”。
...
- 本地版保持 ArchResearch 程序正在运行；网页版保持需要连接的 ArchResearch 标签页处于当前窗口。
