...
1. **M121 模拟用户试点**（in_progress）：用户于 2026-07-27 决定验收改为多 agent 模拟测试、不再要求真人。执行标准仍是 `docs/m121-pilot-kit.md`（任务脚本/判据/分级），盲测 persona 与判据评审分离，结论必须随附模拟局限。试点期间不得截图；persona 发起的 Run 属正常产品使用。
2. M122 表征后模块化 → M123 可重复发布收口（CI、干净机器证明、刷新发布证据；`docs/release-evidence-2026-07-16.md` 与 8 张旧 PNG 引用已删 Run，留在 Git 外等 M123 一并处置，不要当作当前有效证据引用）。
