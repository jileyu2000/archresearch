...
## 2026-07-29 M163 发布结论

- 公共版的小红书核心能力已形成不托管账号的闭环：用户在 Chrome 中保持自己的登录态，扩展只接受公共页的枚举式状态/搜索动作，Worker 只接收最多 8 条已检查且严格清洗的笔记文字、笔记 URL 与图片 URL。
- 进入本地版或公共版主页面都会检查扩展；未检测到时立即提醒，检测到本地 loopback 或公共动态桥后不再弹出。“暂不安装”只关闭当前页面会话，不能伪装成已安装。
- 站外 Release ZIP 仍需解压、开启 Chrome 开发者模式并加载文件夹。普通用户真正的一键安装必须经过 Chrome Web Store 账号、付费与审核，不能由 GitHub Release 替代或伪称已经实现。
...
## 2026-07-30 M165 installation guidance diagnosis

- 用户截图确认线上主按钮“立即安装 Chrome 扩展”直接指向 GitHub Release ZIP；下载动作本身成功，但普通用户看不到解压、开发者模式和“加载已解压的扩展程序”的后续步骤，按钮文案因此错误暗示一键安装。
- 当前 `ResearchComposer` 已有两步概览和 44px 操作区，最小修复不是新增独立页面或第二层弹窗，而是在同一弹窗内做渐进披露：首屏主动作“查看安装方法”，步骤页再显示明确的“下载安装包”。
- Chrome Web Store 尚未发布，不能把站外 ZIP 伪装成一键安装。完整离线流程必须说明选择直接包含 `manifest.json` 的解压文件夹，并在完成后通过扩展工具栏连接当前 ArchResearch 网页。
- `impeccable onboard` 的结论是把用户带到“扩展已连接”的首个价值点，而不是一次性讲完整产品；保留“我已安装，检查连接”和“暂不安装”，只在用户需要安装时展开四步。
- 用户第二张线上截图显示扩展弹窗同时呈现“本地服务已连接”“网页读取已授权”，但“连接当前 ArchResearch 网页”后仍报“未能连接当前网页”，背后的网页安装提醒也未关闭。这说明授权状态、当前标签注入和网页 readiness 握手三者被错误地当成同一状态。
- M165 不能只改文案；必须让当前已打开标签立即获得桥接脚本，并让重复连接保持成功。网页侧在 bridge ready 后应自动关闭提醒，不要求用户刷新或再次点击“我已安装，检查连接”。
- 根因已由代码和线上标签只读诊断确认：公共页 meta 标记正确，第一次连接会完成 exact-origin 注册和当前页注入，但 Board 只在挂载或手动检查时请求状态，没有监听扩展刚注入完成的信号；重复连接又无条件注销并重注册同一个动态脚本，带来无价值的竞态与统一错误文案。
